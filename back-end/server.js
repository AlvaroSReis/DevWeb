...
app.post('/geolocate', (req, res)=> {
  ...
  connection.query(insertQuery, (err, result) => {
    if(result.rows[0].exists == true){
      ...
      longitude = ${user.longitude}, nome = '${user.nome}', cidade = '${user.cidade}' WHERE email = '${user.email}';`)
      res.send('Update sucessful')
    }else{
      connection.query(`INSERT INTO previdencia_social.usuarios(email, latitude, longitude, nome) 
      VALUES('${user.email}', '${user.latitude}', '${user.longitude}', '${user.nome}', cidade = '${user.cidade}');`)
